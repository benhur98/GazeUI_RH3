...
#####################
        roi_l = imutils.resize(frame[y - off:y + off + h + off, x - off:x + w + off], width=250, height=250,
                               inter=cv2.INTER_CUBIC)
        roi_r = imutils.resize(frame[yr - off:yr + off + hr + off, xr - off:xr + wr + off], width=250, height=250,
                               inter=cv2.INTER_CUBIC)
        roi_lo = cv2.resize(roi_l, (250, 250))
        roi_ro = cv2.resize(roi_r, (250, 250))


        roi_lo=roi_lo
        roi_ro=roi_ro

        cv2.putText(roi_lo,str(randint(2000,5000)),(230,230),cv2.FONT_HERSHEY_SIMPLEX,1,198)
        cv2.putText(roi_ro, str(randint(3000, 5000)), (230, 230), cv2.FONT_HERSHEY_SIMPLEX, 1, 198)
        roi_l_resized = cv2.cvtColor(roi_l_resized, cv2.COLOR_BGR2RGB)
        roi_r_resized = cv2.cvtColor(roi_r_resized, cv2.COLOR_BGR2RGB)
        roi_r_resized_rev=np.fliplr(roi_r_resized)

        if count>5:
            
            prediction = model.predict([roi_r_resized.reshape(WIDTH,HEIGHT,3)])
            prediction2=model.predict([roi_r_resized_rev.reshape(WIDTH,HEIGHT,3)])
            ...
            print(int(choice_pred_index) ," -->new " ,int(choice_pred_index2))
            if choice_pred_index == 0 and choice_pred_index2==0:
                pyautogui.moveTo(598,570)
                #pass
                #choice1()
                pass
            elif choice_pred_index == 4 and choice_pred_index2 ==0:
                #right
                pyautogui.moveTo(1203, 597)
                choice2()

                pass
            elif choice_pred_index == 0 and choice_pred_index2 ==4:
                #left
            ...
            cv2.imshow("live feed - 1", roi_lo)
            cv2.imshow("live feed - 2", roi_ro)
            cv2.namedWindow("live feed - 1")
            cv2.namedWindow("live feed - 2")
            cv2.moveWindow("live feed - 1", 185, 20)
            cv2.moveWindow("live feed - 2", 950, 20)

            
        count=count+1
    # cv2.imshow("live feed",frame)
    # cv2.namedWindow("live feed")
    # cv2.moveWindow("live feed",890,400)
    keys = k_check()
